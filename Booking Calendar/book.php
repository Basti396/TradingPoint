...
if(isset($_POST['submit'])){
	...
	foreach ( $timeslots as $number => $timeslot) {
		if($timeslot !== ""){
			...
			if($stmt->execute()){
				...
				$stmt->bind_param('ssssssss', $name, $timeslot, $email, $date, $telefon, $street, $location, $selectOption/*, $order, $nummernschild*/);
					$stmt->execute();
	...
	if(mailSend){
		...
		//Recipients
		$mail->setFrom('termin@trendplace.de', 'Trendplace Team');
		$mail->addAddress($email, $name);     //Add a recipient
		$mail->addAddress('termin@trendplace.de');               //Name is optional
		...
		$body = '<html lang="de">
						...
						<p>Hallo '.$name.',</p>
						<p>hiermit best&auml;tigen wir deinen Termin am '.$date.' um '.$timeslots[0].' in Ravensburg. <br><br>


						<p>Du m&ouml;chtest deinen Termin stornieren? Dann klicke einfach <a href="https://trendplace.de/termine/Calendar_RV/calendar.php?delete=1">hier</a></p>
						<p>Bitte beachte, dass Termine manuell nur ein Tag im voraus storniert werden k&ouml;nnen. Stornierungen am selben Tag m&uuml;ssen telefonisch erfolgen.</p>

						Bei weiteren Fragen, kannst du uns gerne unter folgenden Nummern erreichen: </p>
						<ul>
							<li>WhatsApp: 015259136499</li>
							<li>Im Laden (zu &Ouml;ffnungszeiten): 0751 28502100</li>
						</ul>
						<p>Dein Trendplace Team</p></body></html>';
